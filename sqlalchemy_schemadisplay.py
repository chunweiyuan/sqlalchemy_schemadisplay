"""
This is a template bastardized from the standard installation of
sqlalchemy_schemadisplay,
with modifications in create_schema_graph() to suit our own
visual requirements.

One could either copy this file to the pip install directory (and remove .pyc),
or just import directly from this file in helpers.py
"""
import pydot
import types
...
def _mk_label(mapper, show_operations, show_attributes, show_datatypes,
    ...
    if show_attributes:
        if not show_inherited:
            ...
            cols = mapper.columns
        html +=\
            '<TR><TD ALIGN="LEFT">%s</TD></TR>' % '<BR ALIGN="LEFT"/>'.join(format_col(col) for col in cols)
    else:
    ...
    for mapper in mappers:
        graph.add_node(pydot.Node(escape(mapper.class_.__name__),
                       ...
                       label=_mk_label(mapper, show_operations,
                                       ...
                                       show_inherited, linewidth),
                       fontname=font,
                       fontsize="8.0",))
        if mapper.inherits:
            graph.add_edge(pydot.Edge(escape(mapper.inherits.class_.__name__),
...
def create_schema_graph(tables=None, metadata=None, show_indexes=True,
                        show_datatypes=True, font="Helvetica",
                        concentrate=True, relation_options={}, rankdir='TB'):

    relation_kwargs = {'fontname': font, 'fontsize': "18.0",
                       'fontcolor': "red", 'len': "0.0",
                       'style': "bold"}  # kwargs that define the edges
    ...
    for table in tables:
        for fk in table.foreign_keys:
            if fk.column.table not in tables:
                continue
            edge = [fk.column.table.name, table.name]  # from parent to child
            graph_edge = pydot.Edge(
                label="%s" % fk.column.name,
                arrowhead=(not fk.parent.primary_key and 'odot') or 'dot',
                arrowtail='empty',
